...
async function main() {
  ...
  const DICTToken = await hre.ethers.getContractFactory("DICTToken");
  const DICTTokenContract = await DICTToken.deploy();

  
  console.log(`contract bought successfully at address: ${DICTTokenContract.address}`);
  const receipt = await DICTTokenContract.deployTransaction.wait();
